                ...
                <div id="step-analyze-container" className="space-y-3.5 border-t border-slate-100 pt-4">
                  <div>
                    <label className="text-[11.5px] font-black text-slate-700 uppercase tracking-wider flex items-center justify-between">
                      ...
                      <span className="text-[9px] text-indigo-650 font-bold lowercase font-mono bg-indigo-50 px-2 py-0.5 rounded border border-indigo-150">Continuity Flow</span>
                    </label>
                    <p className="text-[10px] text-slate-400 mt-0.5 leading-normal pb-1.5">{language === 'fr' ? "Scannez votre site internet pour en extraire l'ADN et associez-y votre logotype de marque." : "Scan your URL and associate your company logotype simultaneously."}</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-slate-210 text-slate-800 rounded-lg py-2 pl-8 pr-3 text-xs focus:ring-2 focus:ring-indigo-500/20 focus:outline-none shadow-sm font-semibold"
                      />
                    </div>
                    <button
                      ...
                      className="px-3 py-2 bg-slate-900 hover:bg-black text-white text-xs font-bold rounded-lg hover:shadow-xs transition flex items-center gap-1 cursor-pointer flex-shrink-0 disabled:opacity-50"
                    >
                      {isAnalyzing ? (
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                      ) : (
                  ...
                  {/* PARALLEL LOGO INTEGRATION COMPONENT IN SAME CONTINUITY */}
                  <div className="bg-slate-100 p-3 rounded-xl border border-slate-200/80 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[9.5px] font-extrabold text-slate-500 uppercase tracking-widest block font-mono">
                        🎨 {language === 'fr' ? "Logotype de Marque" : "Brand Logo Transmission"}
                      </span>
                      {settings.logoUrl && (
                        <span className="text-[8px] bg-emerald-100 text-emerald-800 border border-emerald-300 font-black px-1.5 py-0.5 rounded uppercase font-mono">
                    ...
                    {settings.logoUrl ? (
                      <div className="flex items-center gap-3 bg-white border border-slate-150 p-2 rounded-lg">
                        <div className="relative flex items-center justify-center bg-white border border-slate-200 rounded-md p-1 w-10 h-10 shadow-3xs flex-shrink-0 select-none overflow-hidden">
                          <img 
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          <p className="text-[10px] font-extrabold text-slate-800 truncate">{language === 'fr' ? "Logotype Actif" : "Active Brand Logo"}</p>
                          <p className="text-[8px] text-slate-400 truncate">{language === 'fr' ? "Incorpore l'identité sur les slides" : "Embeds identity on all story frames"}</p>
                        </div>
                        <button 
                          type="button"
                          onClick={() => onUpdateSettings({ ...settings, logoUrl: "" })}
                          className="p-1 bg-red-50 hover:bg-red-100 text-red-650 rounded-md transition cursor-pointer"
                      ...
                      <div 
                        id="continuity-logo-drag-container"
                      ...
                      >
                        <input
                          ref={fileInputRef}
                          id="continuity-logo-file-input"
                          type="file"
                          accept="image/png, image/jpeg, image/jpg"
                          onChange={handleLogoInputChange}
                          className="hidden"
                        />
                        <Upload className="w-5 h-5 text-indigo-500 mx-auto mb-1 animate-bounce" />
                        <p className="text-[10px] font-black text-slate-700">{t.logo_drag_placeholder}</p>
                        <p className="text-[8.5px] text-slate-450 mt-0.5">{language === 'fr' ? "Mettez votre logo pour l'intégrer automatiquement" : "Transmit logo here to couple with website content"}</p>
                      </div>
                    )}

                    <div className="flex justify-between items-center bg-white/40 p-1.5 rounded-lg text-[9px]">
                      <span className="text-slate-450 font-medium">{language === 'fr' ? "Format PNG avec fond transparent recommandé" : "Transparent background is recommended."}</span>
                      <button
                        type="button"
                        onClick={() => setShowUrlLogoInput(!showUrlLogoInput)}
                        className="text-indigo-650 font-black hover:underline cursor-pointer"
                      >
                        {showUrlLogoInput ? "Fermer" : "Logo URL"}
                      </button>
                    </div>
                    ...
                    {showUrlLogoInput && (
                      <div className="pt-1 select-text">
                        <input
                          id="continuity-input-logo-url"
                          type="text"
                          placeholder={t.logo_url_placeholder}
                          value={settings.logoUrl || ""}
                          onChange={(e) => onUpdateSettings({ ...settings, logoUrl: e.target.value })}
                          className="w-full bg-white border border-slate-205 text-slate-805 rounded-md py-1.5 px-2.5 text-[10px] focus:ring-1 focus:ring-indigo-500 font-medium select-text"
                        />
                      </div>
                    )}
                  </div>
